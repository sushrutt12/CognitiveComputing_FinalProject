...
from keras.layers import Dense, Activation, Flatten, Convolution2D, Permute, Convolution1D, Input, concatenate
...
# Create networks for DQN
# Next, we build a very simple model.

actor = Sequential()
actor.add(Flatten(input_shape=(1,) + env.observation_space.shape))
#(batch_size, steps, input_dim)
#actor.add(Convolution2D(32, 8, 8))
...
#                  delta_clip=1.)
#(model, policy=None, test_policy=None, enable_double_dqn=False, enable_dueling_network=False, dueling_type='avg')
agent = DQNAgent(actor,nb_actions=env.noutput, memory=memory, nb_steps_warmup=100, enable_double_dqn=False,
                            gamma=.99, target_model_update=1e-3)

#agent = DQNAgent(nb_actions=env.noutput, V_model=V_model, L_model=L_model, mu_model=mu_model,
